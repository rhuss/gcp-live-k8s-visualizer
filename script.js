    ...
    for (var i = 0; i < controllers.items.length; i++) {
        ...
        for (var j = 0; j < pods.items.length; j++) {
            ...
            if (pod.metadata.labels.name == controller.metadata.labels.name) {
                if (controller.metadata.labels.version &&
                    pod.metadata.labels.version &&
                    (controller.metadata.labels.version != pod.metadata.labels.version)) {
                    continue;
                }
...
var renderNodes = function () {
    ...
    $.each(nodes.items, function (index, value) {
        ...
        var eltDiv = $(
          '<div class="window node ' + ready + '" title="' + value.metadata.name + '" id="node-' + value.metadata.name +
    ...
    $.each(groupOrder, function (ix, key) {
        ...
        $.each(list, function (index, value) {
            ...
            if (value.type == "pod") {
                ...
                eltDiv = $(
                  '<div class="window pod ' + phase + '" title="' + value.metadata.name + '" id="pod-' + value.metadata.name +
                ...
                eltDiv = $(
                  '<div class="window wide service ' + phase + '" title="' + value.metadata.name + '" id="service-' + value.metadata.name +
                  '" style="left: ' + 75 + '; top: ' + y + '"/>');
                eltDiv.html('<span>' +
                            value.metadata.name +
                ...
                eltDiv = $(
                  '<div class="window wide controller" title="' + value.metadata.name + '" id="controller-' + value.metadata.name +
...
var loadData = function () {
    ...
    var req1 = $.getJSON("/api/v1/pods?labelSelector=" + TL_FILTER, function (data) {
        pods = data;
    ...
    var req2 = $.getJSON("/api/v1/replicationcontrollers?labelSelector=" + TL_FILTER, function (data) {
        controllers = data;
        $.each(data.items, function (key, val) {
    ...
    var req3 = $.getJSON("/api/v1/services?labelSelector=" + TL_FILTER, function (data) {
        services = data;
